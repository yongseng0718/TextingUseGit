...
if ($user_id) {
    ...
    foreach ($cart_items as $product_variant_id => $item) {
        ...
        $updated_cart[$product_variant_id] = array_merge($item, [
            ...
            'discount'    => $extra_data['discount'] ?? $item['discount'],
            'image_url'   => $extra_data['image_url'] ?? 'default.jpg',
            'color'       => $extra_data['color'] ?? '',
            'name'=> $extra_data['name'] ?? 'Unknown Product'
        ]);        

        $discounted_price =  $updated_cart[$product_variant_id] ['price'] * (1 -  $updated_cart[$product_variant_id]['discount'] / 100);
        $total_price += $discounted_price * $updated_cart[$product_variant_id]['quantity'];
    }
